...
export class Scrapping {
  async exec(container: string, mbl: string) {
    ...
    const result = await page.evaluate(({ container, mbl }) => {
      const vesselsCollapse = document.querySelector("[data-test='vessels_label'] span") as HTMLElement;
      const otherReferencesCollapse = document.querySelector(
        "[data-test='other_references_label'] span"
      ) as HTMLElement;
      const shipmentHistoryCollapse = document.querySelector(
        "[data-test='shipment_status_history_label'] span"
      ) as HTMLElement;
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          const shipmentInformation = Array.from(document.querySelectorAll(".tracking-details-value")).map((element) =>
            element.textContent.trim()
          );

          const [vesselName] = Array.from(document.querySelectorAll("td[data-test*='vessels']")).map((el) =>
            el.textContent.trim()
          );

          const shipmentHistorySection = Array.from(
            document.querySelectorAll("td[data-test*='shipment_status_history']")
